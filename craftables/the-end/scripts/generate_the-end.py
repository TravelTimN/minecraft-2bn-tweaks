...
NAMESPACE = "pre_end"
PACK_DESCRIPTION = "§42§cB§6N§f-§eT§aw§be§9a§5k§ds §f> §eCraftables §f: §bPre-End Recipes"
BASE_PACK_FORMAT = 15
...
def build_recipe(recipe_def, fmt):
    if fmt == "flat":
        key = {
            key_name: f"minecraft:{item}"
            for key_name, item in recipe_def["ingredients"].items()
        }
        result = {
            "id": f"minecraft:{recipe_def['result']}",
            "count": recipe_def["count"]
        }
    else:
        key = {
            key_name: {"item": f"minecraft:{item}"}
            for key_name, item in recipe_def["ingredients"].items()
        }
        result_key = "id" if fmt == "id" else "item"
        result = {
            result_key: f"minecraft:{recipe_def['result']}",
            "count": recipe_def["count"]
        }

    return {
        "type": "minecraft:crafting_shaped",
        "group": recipe_def["group"],
        "pattern": recipe_def["pattern"],
        "key": key,
        "result": result
    }
...
for pack in all_packs:
    ...
    for recipe_def in RECIPES:
        filepath = os.path.join(output_path, f"{recipe_def['name']}.json")
        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(build_recipe(recipe_def, fmt), f, indent=2)
        count += 1
...
for pack, count in recipe_counts.items():
    ...
    print(f"  - {label:<12}: recipes created = {count}")
